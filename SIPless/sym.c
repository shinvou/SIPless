...
uintptr_t find_sym(uintptr_t base, const char *name)
{
    ...
    if(!stab)
    {
        ...
        if(!stab)
        {
            LSI("Failed to find LC_SYMTAB");
            return 0;
        }
        LSI("symtab offset: 0x%08x", stab->symoff);
        LSI("strtab offset: 0x%08x", stab->stroff);
    }
    ...
    if(!symtab || !strtab)
    {
        ...
        if(!symtab)
        {
            LSI("Failed to find symtab");
            return 0;
        }
        if(!strtab)
        {
            LSI("Failed to find strtab");
            return 0;
        }
    }
    ...
    LSI("Failed to find symbol: %s", name);
    return 0;
}
